...
erDiagram
    ...
    USERS ||--o| NOTIFICATION_PREFERENCES : "has at most one"
    ...
    RECOVERY_CASES ||--o| DEVICE_RECOVERY_CHECKLISTS : "has at most one"
    ...
    DEVICE_RECOVERY_CHECKLISTS {
        uuid id PK
        uuid case_id FK UK "one per case"
        device_recovery_checklist_item_array completed_items
        timestamptz recovered_at "independent of recovery_cases.closed_at"
        timestamptz closed_at
    }
    NOTIFICATIONS {
        uuid id PK
    ...
    NOTIFICATION_PREFERENCES {
        uuid id PK
        uuid user_id FK UK "one per user, not per case"
        notification_type_array muted_types
        bool quiet_hours_enabled
        smallint quiet_hours_start_minute
        smallint quiet_hours_end_minute
        text timezone
    }
    AUDIT_EVENTS {
        uuid id PK
