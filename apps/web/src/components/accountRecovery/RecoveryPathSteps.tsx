...
const SPEED_STYLES: Record<AccountRecoveryStep['speed'], string> = {
  FAST: 'border-emerald-900 bg-emerald-950 text-emerald-300',
  VARIES: 'border-amber-900 bg-amber-950 text-amber-300',
  SLOW: 'border-red-900 bg-red-950 text-red-300',
};
    ...
    <div className="rounded-lg border border-slate-800 bg-slate-900 p-5">
      ...
      <ol className="mt-3 space-y-3">
        {steps.map((step, index) => (
          <li key={step.key} className="rounded-md border border-slate-800 p-3">
            ...
            {step.officialExternalAction?.url ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-flex items-center text-xs font-medium text-sky-400 hover:underline"
              >
