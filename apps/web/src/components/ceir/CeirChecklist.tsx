    ...
    <div className="glass-panel p-5">
      ...
      <ul className="mt-3 space-y-2">
        {hints.map((hint) => {
          ...
          return (
            <li key={hint.item} className="flex items-start gap-2.5 rounded-md border border-white/10 p-2.5">
              <input
                type="checkbox"
                checked={checked}
                disabled={submitting}
                onChange={(e) => void onToggle(hint.item, e.target.checked)}
                className="mt-0.5 h-4 w-4 rounded border-white/20 bg-white/5 text-emerald-600 focus:ring-emerald-600"
              />
              <div>
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-white">{hint.label}</p>
                  {hint.satisfied ? (
                    <span className="rounded-full border glass-panel-success px-1.5 py-0.5 text-[10px] font-semibold text-emerald-300">
                      on file
                    </span>
                  ) : null}
                </div>
